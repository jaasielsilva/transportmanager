...
| carga (CRUD ref) | ✅ back e front (lista + form, quota, soft delete) + **estimativa de rota**: Matrix do OpenRouteService via `features/geo` (`POST /cargas/calcular-rota`), API key só no backend (`OPENROUTESERVICE_API_KEY`, plano grátis sem cartão), falha do upstream → 502 |
